...
def main():
    ...
    plt.yscale("log")
    plt.title("Moyennes du temps d'exécution des algorithmes de backtracking")
    plt.ylabel("Secondes")
...
def chance_g():
    ...
    plt.legend(loc='upper left')
    plt.axhline(size*size,linestyle="--")
    plt.ylim(0,size*size+5)
    plt.ylabel("Cases trouvées")
    plt.xlabel("Probabilité qu'une case soit noire (en %)")
    plt.title("Moyennes des cases trouvées par les règles logiques dans une grille de taille " + str(size))
    plt.show()

def comprar():
    all_data = [g_data(x) for x in backtrack_lr_f]
    all_data_lr = [g_data(x) for x in backtrack_nd_lr_f]
    ...
    n =list(map(str, range(4,21)))
    x = np.arange(len(n))
    width = 0.40
    offset = width
    plt.cla()
    plt.bar(x+offset,time_data,width,label="Backtracking avec automates déterministes et règles logiques",color="blue")
    offset += offset
    plt.bar(x+offset,time_data_lr,width,label="Backtracking avec automates non-déterministes et règles logiques",color="orangered")
    plt.yscale("log")
    plt.title("Comparaison du temps d'exécution (médianes)")
    plt.ylabel("Secondes")
    plt.xlabel("Taille du graphe")
